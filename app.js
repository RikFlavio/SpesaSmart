...
const renderDetail=(p,cat,api)=>{console.log('renderDetail',p);let h=`<div class="detail-head"><div class="detail-img">${p.image?`<img src="${p.image}">`:(cat.emoji)}</div><div class="detail-info"><h3>${p.name}</h3><p>${p.barcode||''}</p></div></div>`;
h+=`<div class="detail-section"><h4>💰 Prezzi</h4><div id="pricesList">`;
if(p.prices&&Object.keys(p.prices).length){Object.entries(p.prices).forEach(([sid,price])=>{const shop=shops.find(s=>s.id===sid);if(shop)h+=`<div class="price-row"><span>${shop.emoji} ${shop.name}</span><span>€ ${price.toFixed(2)}</span><button onclick="delPrice('${p.id}','${sid}')">✕</button></div>`})}else{h+=`<div class="no-data">Nessun prezzo inserito</div>`}
h+=`</div><button class="btn-link" onclick="addPrice('${p.id}')">+ Aggiungi prezzo</button></div>`;
if(api){const ns=api.nutriscore_grade||api.nutrition_grades;if(ns)h+=`<div class="detail-section"><h4>Qualità</h4><div class="nutri-row"><div class="nutri-badge ${ns}">${ns.toUpperCase()}</div><span>Nutri-Score</span></div></div>`;const n=api.nutriments||{};const vals=[{l:'Energia',v:n['energy-kcal_100g'],u:'kcal'},{l:'Grassi',v:n.fat_100g,u:'g'},{l:'Carboidrati',v:n.carbohydrates_100g,u:'g'},{l:'Proteine',v:n.proteins_100g,u:'g'}].filter(x=>x.v!=null);if(vals.length)h+=`<div class="detail-section"><h4>Per 100g</h4><div class="info-grid">${vals.map(x=>`<div class="info-item"><small>${x.l}</small><strong>${Number(x.v).toFixed(0)} ${x.u}</strong></div>`).join('')}</div></div>`;const{quantity,serving_size}=api;if(quantity||serving_size)h+=`<div class="detail-section"><h4>Confezione</h4>${quantity?`<div class="pkg-row"><span>Quantità</span><span>${quantity}</span></div>`:''}${serving_size?`<div class="pkg-row"><span>Porzione</span><span>${serving_size}</span></div>`:''}</div>`}h+=`<button class="btn-outline" onclick="delProd('${p.id}')">🗑️ Elimina</button>`;$('detailBody').innerHTML=h};
...
window.addPrice=async id=>{const p=products.find(x=>x.id===id);if(!p)return;if(!shops.length){toast('Crea prima un supermercato');return}const shopId=await selectShop();if(!shopId)return;const price=await dialog('Prezzo (€)',true);if(price){const val=parseFloat(price.replace(',','.'));if(!isNaN(val)&&val>0){if(!p.prices)p.prices={};p.prices[shopId]=val;await dbPut('products',p);openDetail(id)}}};
window.delPrice=async(id,sid)=>{const p=products.find(x=>x.id===id);if(p&&p.prices){delete p.prices[sid];await dbPut('products',p);openDetail(id)}};
const selectShop=()=>new Promise(res=>{let h='<p style="margin-bottom:12px">Seleziona supermercato:</p><div class="shop-select">';shops.forEach(s=>{h+=`<button class="shop-sel-btn" data-id="${s.id}">${s.emoji} ${s.name}</button>`});h+='</div>';$('detailBody').innerHTML=h;$('detailBody').querySelectorAll('.shop-sel-btn').forEach(b=>{b.onclick=()=>res(b.dataset.id)})});
...
// Export/Import
const exportData=()=>{const d={v:1,date:new Date().toISOString(),products:products.map(p=>({id:p.id,name:p.name,cat:p.cat,barcode:p.barcode,image:p.image,shops:p.shops,prices:p.prices||{},qty:p.qty,done:p.done})),shops:shops.map(s=>({id:s.id,name:s.name,emoji:s.emoji}))};const b=new Blob([JSON.stringify(d,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`spesasmart-${new Date().toISOString().split('T')[0]}.json`;a.click();toast('Esportato')};
const importData=async f=>{try{const d=JSON.parse(await f.text());if(!d.products)throw 0;await dbClear('products');for(const p of d.products)await dbPut('products',p);if(d.shops){await dbClear('shops');for(const s of d.shops)await dbPut('shops',s)}await load();renderFilters();renderList();closeAll();toast('Importato')}catch(e){toast('Errore')}};
