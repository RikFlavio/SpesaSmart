...
const renderDetail=(p,cat,api)=>{let h=`<div class="detail-head"><div class="detail-img">${p.image?`<img src="${p.image}">`:(cat.emoji)}</div><div class="detail-info"><h3>${p.name}</h3><p>${p.barcode||''}</p></div></div>`;
// Prezzi per supermercato
h+=`<div class="detail-section"><h4>Prezzi</h4><div id="pricesList">`;
if(p.prices&&Object.keys(p.prices).length){Object.entries(p.prices).forEach(([sid,price])=>{const shop=shops.find(s=>s.id===sid);if(shop)h+=`<div class="price-row"><span>${shop.emoji} ${shop.name}</span><span>€ ${price.toFixed(2)}</span><button onclick="delPrice('${p.id}','${sid}')">✕</button></div>`})}else{h+=`<div class="no-data">Nessun prezzo</div>`}
